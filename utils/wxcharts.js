...
var config = {
    ...
    colors: ['#fd0002', '#ff9900', '#00af43', '#04c8f9', '#04c8f9', '#8085e9'],
...
function calLegendData(series, opts, config) {
    ...
    var currentRow = [];
    series.forEach(function (item) {
        var itemWidth = 10 * padding + shapeWidth + measureText(item.name || 'undefined');
        if (widthCount + itemWidth > opts.width) {
          legendList.push(currentRow);
          widthCount = itemWidth + padding;
          currentRow = [item];
        } else {
          widthCount += itemWidth + padding;
...
function drawToolTip(textList, offset, opts, config, context) {
    ...
    var toolTipWidth = legendWidth + legendMarginRight + config.toolTipPadding + Math.max.apply(null, textWidth);
